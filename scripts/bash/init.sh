...
if $JSON_MODE; then
    ...
    echo ""
    if [ "$IS_LARAVEL" = true ]; then
        echo "[ds] Initialized DevSquad SDD in $TARGET_DIR (Laravel + Boost)"
        echo ""
        echo "  .ai/skills/ds-*/        $SKILLS_COUNT skills (Laravel Boost format)"
        ...
        echo ""
        echo "Next steps:"
        echo "  1. Run: php artisan boost:update"
        echo "  2. Run /ds.constitution to set up your project principles"
        echo "  3. Run /ds.specify <JIRA-CODE> to create your first feature spec"
    else
